...
class Player(Entity):
    def __init__(self, image, x, y, player_lives):
        ...
        self.invisible_time = UNTARGETABILITY_TIME

    def hit(self):
        self.lives -= 1
        self.X = INITIAL_PLAYER_X1
        self.Y = INITIAL_PLAYER_Y1
        self.exploding()
        if self.lives == 0:
            self.dead()
            return True
    ...
    def explosion(self, screen):
        ...
        if self.exploding_time <= 0:
            self.exploding_time = PLAYER_EXPLODING_TIME
            self.untargetability()

    def untargetability(self):
        self.player_state = PlayerState.UNTARGETABILITY
...
class Rocket(Entity):
    ...
    def exploding(self, explode_image, screen):
        self.rocket_state = RocketState.EXPLODING
        screen.blit(pygame.image.load(explode_image), (self.X, self.Y))
