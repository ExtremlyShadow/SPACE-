...
class PlayerGamePlay(BaseGamePlay):
    ...
    def i_frames(self):
        if self.player.player_state == PlayerState.UNTARGETABILITY:
            self.player.invisible_time -= 1
            self.game_setting.screen.blit(pygame.image.load(UNTARGETABILITY_IMAGE),
                                          ((2 * self.player.X + IMAGE_SIZE) / 2,
                                          (2 * (HEIGHT - self.player.Y) + IMAGE_SIZE)/2))
        if self.player.invisible_time == 0:
            self.player.player_state = PlayerState.PLAYING
            self.player.invisible_time = UNTARGETABILITY_TIME
